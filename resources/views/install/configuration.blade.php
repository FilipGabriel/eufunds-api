    ...
    <form method="POST" action="{{ route('install.configuration.post') }}" class="form-horizontal">
        ...
        <div class="box">
            ...
            <div class="configure-form p-b-0">
                ...
                <div class="form-group {{ $errors->has('app.app_email') ? 'has-error': '' }}">
                    <label class="control-label col-sm-3" for="app-email">App Email <span>*</span></label>

                    <div class="col-sm-9">
                        <input type="text" name="app[app_email]" value="{{ old('app.app_email') }}" id="app-email" class="form-control">

                        {!! $errors->first('app.app_email', '<span class="help-block">:message</span>') !!}
                    </div>
                </div>

                <div class="form-group {{ $errors->has('app.app_phone') ? 'has-error': '' }}">
                    <label class="control-label col-sm-3" for="app-phone">App Phone <span>*</span></label>

                    <div class="col-sm-9">
                        <input type="text" name="app[app_phone]" value="{{ old('app.app_phone') }}" id="app-phone" class="form-control">

                        {!! $errors->first('app.app_phone', '<span class="help-block">:message</span>') !!}
                    </div>
                </div>

                <div class="form-group {{ $errors->has('app.search_engine') ? 'has-error': '' }}">
                    <label class="control-label col-sm-3" for="app-search-engine">Search Engine <span>*</span></label>

                    <div class="col-sm-9">
                        <select name="app[search_engine]" class="form-control custom-select-black" id="app-search-engine">
                            <option value="mysql" {{ old('app.search_engine') === 'mysql' ? 'selected' : '' }}>
                                MySQL
                            </option>

                            <option value="algolia" {{ old('app.search_engine') === 'algolia' ? 'selected' : '' }}>
                                Algolia
                            </option>
                        </select>

                        @if ($errors->has('app.search_engine'))
                            {!! $errors->first('app.search_engine', '<span class="help-block">:message</span>') !!}
                        @else
                ...
                <div class="{{ old('app.search_engine') === 'algolia' ? '' : 'hide' }}" id="algolia-fields">
                    <div class="form-group {{ $errors->has('app.algolia_app_id') ? 'has-error': '' }}">
                        <label class="control-label col-sm-3" for="app-algolia-app-id">Algolia Application ID <span>*</span></label>

                        <div class="col-sm-9">
                            <input type="text" name="app[algolia_app_id]" value="{{ old('app.algolia_app_id') }}" id="app-algolia-app-id" class="form-control">

                            {!! $errors->first('app.algolia_app_id', '<span class="help-block">:message</span>') !!}
                        </div>
                    </div>

                    <div class="form-group {{ $errors->has('app.algolia_secret') ? 'has-error': '' }}">
                        <label class="control-label col-sm-3" for="app-algolia-secret">Algolia Admin API Key <span>*</span></label>

                        <div class="col-sm-9">
                            <input type="password" name="app[algolia_secret]" value="{{ old('app.algolia_secret') }}" id="app-algolia-secret" class="form-control">

                            {!! $errors->first('app.algolia_secret', '<span class="help-block">:message</span>') !!}
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        $('#app-search-engine').on('change', function () {
