    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh("rm -rf ${STORAGE_SYM}/")
                sh("rm -f ${ZIP_FILE}")
                script {
                    zip zipFile: "${ZIP_FILE}", archive: true, dir: '.'
                }
                sh("scp ${SRC_PATH}/${ZIP_FILE} ${DEST_HOST_USER}@${DEST_HOST}:${DEST_PATH}")
                // unzip build
                sh("""ssh ${DEST_HOST_USER}@${DEST_HOST} '''
                    cd ${DEST_PATH};
...
//                     chown jenkins:www-data bootstrap/cache/routes-v7.php;
                ...
                sh("ssh ${DEST_HOST_USER}@${DEST_HOST} 'cd ${DEST_PATH}; ls -lt | tail -n +7 | awk '\\''{print \$9}'\\'' | xargs -r rm -r'")
            }
        }
    }
    post {
