...
$('#vat_enabled').on('change', () => {
    $('#vat-value').toggleClass('hide');
});
...
$('#app_country').on('change', (e) => {
    let oldState = $('#app_state').val();

    $.ajax({
        type: 'GET',
        url: route('countries.states.index', e.currentTarget.value),
        success(states) {
            $('.app-state').addClass('hide');

            if (_.isEmpty(states)) {
                return $('.app-state.input').removeClass('hide').find('input').val(oldState);
            }
            ...
            $('.app-state.select').removeClass('hide')
                .find('select')
                .html(options)
                .val(oldState);
        },
