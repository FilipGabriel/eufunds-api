...
class Coupon extends Model
{
    ...
    public function applicableUsers()
    {
    ...
    public function inapplicableUsers()
    {
        if($this->excludeUsers->isEmpty()) {
            return false;
        }

        return $this->excludeUsers->contains(auth()->id());
    }
    ...
    {
        return $this->belongsToMany(Program::class, 'coupon_programs')
            ->withPivot('exclude')
            ->wherePivot('exclude', false);
    }

    public function excludePrograms()
    {
        return $this->belongsToMany(Program::class, 'coupon_programs')
            ->withPivot('exclude')
            ->wherePivot('exclude', true);
    ...
    public function getCalculatedValue($subTotal)
    {
        if (! $this->is_percent) {
            return $this->value;
        }

        return $subTotal->divide(100 / $this->value);
    }
    ...
    {
        $this->syncUsers(array_get($attributes, 'users', []));
        $this->syncExcludeUsers(array_get($attributes, 'exclude_users', []));
        ...
        $this->syncPrograms(array_get($attributes, 'programs', []));
        $this->syncExcludePrograms(array_get($attributes, 'exclude_programs', []));
    }
}
